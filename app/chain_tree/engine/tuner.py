...
class DataTuner:
    def __init__(
        self, dataset_loader: DatasetLoader, verbose: bool = False, client=None
    ):
    ...
    def wait_for_file_ready(self, file_id, timeout=600, poll_interval=10):
        """
        Polls Groq's servers for the file's status until it's ready or until the timeout.
    ...
    def process_and_fine_tune(
        ...
        jsonl_output_filename_train = f"{output_filename}_train.jsonl"
        log_handler(
        ...
        log_handler(
            "Uploading training file to Groq...", level="info", verbose=self.verbose
        )
    ...
    def fine_tune_and_return(
        ...
        """
        Fine-tunes a language model based on a DataFrame and returns the fine-tuned model name.
        """
        output_filename = f"{self.dataset_loader.output_directory}/fine_tuning_data"
        return self.process_and_fine_tune(
            output_filename,
            model_suffix,
            system_message_text,
            target_model,
