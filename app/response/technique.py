...
from app.response.interface import ChainBuilder
from app.response.interface import Technique
...
class SynthesisTechniqueDirector(Technique):
    ...
    def _generate_prompt(
        ...
        """
        prompt_data = self.technique.prompts.get(prompt, None)
        prompt_template = prompt_data.get("template") if prompt_data else None
        ...
        if prompt_template:
            ...
            # If a template is not provided or prompts is None, fallback to the description
            prompt_text = self.technique.description
    ...
    def _build_synthesis(self, coordinate: Optional[ChainCoordinate] = None):
        ...
        prompt_data = self.technique.prompts.get(selected_prompt)

        if not prompt_data:
            # If no prompt data is found, use the technique description
            extended_prompt = self.technique.description
        else:
            # Get the branching options and dynamic prompts for the selected prompt
            branching_options = prompt_data.get("branching_options", [])
            dynamic_prompts = prompt_data.get("dynamic_prompts", [])

            if not branching_options or not dynamic_prompts:
                # If no branching options or dynamic prompts, use the technique description
                extended_prompt = self.technique.description
            else:
