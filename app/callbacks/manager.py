...
from app.chain_tree.base import EntityAction, EntityFinish, LLMResult, BaseMessage
from app.services.generation_service import get_buffer_string
...
class CallbackManagerForChainRun(RunManager, ChainManagerMixin):
    ...
    def on_agent_finish(self, finish: EntityFinish, **kwargs: Any) -> Any:
...
class AsyncCallbackManagerForChainRun(AsyncRunManager, ChainManagerMixin):
    ...
    async def on_agent_action(self, action: EntityAction, **kwargs: Any) -> Any:
        """Run when agent action is received."""
        await _ahandle_event(
            self.handlers,
            "on_agent_action",
    ...
    async def on_agent_finish(self, finish: EntityFinish, **kwargs: Any) -> Any:
        """Run when agent finish is received."""
        await _ahandle_event(
            self.handlers,
            "on_agent_finish",
